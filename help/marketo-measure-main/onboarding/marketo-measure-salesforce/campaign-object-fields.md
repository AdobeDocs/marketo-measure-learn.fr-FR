---
title: Champs d’objet de campagne
description: Cette leçon couvre les champs d’objet de campagne CRM Salesforce que  [!DNL Marketo Measure]  utilisez pour mapper les campagnes et les membres de campagne aux canaux marketing. Il est important de comprendre le rôle de ces objets Campaign lorsque vous gérez et mappez vos canaux hors ligne dans  [!DNL Marketo Measure].
role: User
level: Beginner
doc-type: Feature Video
last-substantial-update: 2023-01-06T00:00:00Z
jira: KT-11690
thumbnail: 347238.jpeg
exl-id: 72a094c9-2659-4cb2-8c6e-7d9187ef3a03
feature: Salesforce
source-git-commit: 557fe2c83a26425c5c849e6260a416215ce1c7fb
workflow-type: tm+mt
source-wordcount: '77'
ht-degree: 0%

---

# Champs d’objet de campagne

Cette leçon couvre les champs d’objet de campagne CRM Salesforce que [!DNL Marketo Measure] utilise pour mapper les campagnes et les membres de campagne aux canaux marketing. Il est important de comprendre le rôle de ces objets Campaign lorsque vous gérez et mappez vos canaux hors ligne dans [!DNL Marketo Measure].

>[!VIDEO](https://video.tv.adobe.com/v/347238/?learn=on){transcript=true}
